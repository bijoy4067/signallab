...
class NotificationController extends Controller
{
    ...
    public function globalUpdate(Request $request){
        $request->validate([
            ...
            'sms_body' => 'required',
            'firebase_template' => 'required',
            'telegram_template' => 'required',
        ]);
        ...
        $general->firebase_template = $request->firebase_template;
        $general->telegram_template = $request->telegram_template;
        $general->save();
    ...
    public function templateUpdate(Request $request,$id){
        ...
        $template->firebase_status = $request->firebase_status ? Status::ENABLE : Status::DISABLE;
        $template->telegram_body = $request->telegram_body;
        $template->telegram_status = $request->telegram_status ? Status::ENABLE : Status::DISABLE;
        $template->save();
   ...
   public  function smsSetting(){
    ...
    public function pushSettingUpdate(Request $request){
        ...
        try{
            ...
            $config = "var firebaseConfig = ".json_encode($general->firebase_config);
    ...
    public function telegramSetting(){
        ...
        return view('admin.notification.telegram_setting', compact('pageTitle'));
    }

    public function telegramSettingUpdate (Request $request)
    {
        $request->validate([
        ...
        $general = gs();
        $general->telegram_config = [
            'bot_api_token'=>$request->bot_api_token,
