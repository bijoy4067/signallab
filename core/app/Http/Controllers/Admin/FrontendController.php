...
class FrontendController extends Controller
{
    ...
    public function frontendContent(Request $request, $key)
    {
        ...
        $videoJson = @getPageSections()->$key->$type->videos;
        ...
        foreach ($request->except('_token', 'video') as $input_field => $val) {
            ...
            elseif ($input_field == 'has_video' && $videoJson) {
                foreach ($videoJson as $videoValKey => $videoJsonVal) {
                    $validation_rule['video_input.' . $videoValKey] = ['nullable', 'mimes:mp4,ogx,oga,ogv,ogg,webm'];
                    $validation_message['video_input.' . $videoValKey . '.video'] = keyToTitle($videoValKey) . ' must be an image';
                    $validation_message['video_input.' . $videoValKey . '.mimes'] = keyToTitle($videoValKey) . ' file type not supported';
                }
                continue;
            }
        ...
        if($videoJson){
            foreach ($videoJson as $videoKey => $videoValue) {
                $videoData = @$request->video_input[$videoKey];
                if (is_file($videoData)) {
                    try {
                        $inputContentValue[$videoKey] = fileUploader($videoData, 'assets/images/frontend/' . $key, null, @$content->data_values->$videoKey);
                    } catch (\Exception $exp) {
                        $notify[] = ['error', 'Couldn\'t upload the file'];
                        return back()->withNotify($notify);
                    }
                } else if (isset($content->data_values->$videoKey)) {
                    $inputContentValue[$videoKey] = $content->data_values->$videoKey;
                }
            }
        }
    ...
    public function importContent(Request $request, $key)
    {
        ...
        $toTemp   = $general->active_template;
        ...
        if (@$toTempJson['element']) {
            ...
            $previousElements = Frontend::where('template_name', $toTemp)->where('data_keys', $key . '.element')->get();
