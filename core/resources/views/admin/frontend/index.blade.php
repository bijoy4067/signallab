                                ...
                                @foreach($section->content as $k => $item)
                                    ...
                                    @else
                                        @if($k != 'images')
                                            ...
                                            @elseif($k == 'videos')
                                                @php
                                                    $videoCount = collect($item)->count();
                                                @endphp
                                                @foreach($item as $videoKey => $video)
                                                    <div class="col-md-12">
                                                        <div class="form-group">
                                                            <label>{{__(keyToTitle(@$videoKey))}}</label>
                                                            <div class="input-group">
                                                                <input type="hidden" name="has_video" value="1">
                                                                <input type="file" class="form-control rounded" name="video_input[{{ @$videoKey }}]">
                                                                <div
                                                                    class="input-group-text ms-2 btn btn--primary align-itmes-center d-flex rounded viewVideo"
                                                                    data-src="{{ asset('assets/images/frontend/banner/'. @$content->data_values->$videoKey) }}"
                                                                    data-data="{{ @$content->data_values->$videoKey ?? null }}"
                                                                >
                                                                    @lang('View')
                                                                </div>
                                                            </div>
    ...
    <x-confirmation-modal />
        ...
        <div id="videoModal" class="modal fade" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">@lang('Video')</h5>
                        <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                            <i class="las la-times"></i>
                        </button>
                    </div>
...
<div class="import-parent d-flex flex-wrap justify-content-end gap-2 align-items-center">
    @if(@$section->element)
        <div class="newBtn">
            @if($section->element->modal)
    ...
    <div class="form-inline float-sm-end">
        <form action="{{ route('admin.frontend.import',$key) }}" method="post">   
            <div class="input-group custom-import">
                @csrf
    ...
    <script>
        (function ($) {
            ...
            $('.viewVideo').on('click', function () {
                ...
                if(!$(this).data('data')){
                    videoHtml = `<span class='text-center d-block'>{{ __($emptyMessage) }}</span>`;
                }
                else{
                    videoHtml = `
                        <video height='262' max-width='100%' controls autoplay>
